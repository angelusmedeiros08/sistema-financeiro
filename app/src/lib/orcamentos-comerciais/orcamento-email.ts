...
export async function enviarEmailOrcamento(params: {
  ...
  if ("erro" in config) {
    if (process.env.NODE_ENV !== "production") {
      console.warn(`[orcamento-email] SMTP não configurado — link do orçamento #${params.numeroOrcamento} pra ${params.email}:\n${params.link}`);
      return { sucesso: true };
    }
    return config;
  }
  ...
  const assunto = params.atualizado
    ? `Orçamento #${params.numeroOrcamento} atualizado — ${params.tenantNome}`
    : `Orçamento #${params.numeroOrcamento} — ${params.tenantNome}`;
  const abertura = params.atualizado
    ? `A proposta <strong>#${params.numeroOrcamento}</strong> de <strong>${tenantNome}</strong> foi atualizada.`
    ...
    await transportador.sendMail({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
          <h2 style="color: #111827;">Orçamento #${params.numeroOrcamento}</h2>
