...
import { criarVenda, editarCabecalhoVenda, enviarOrcamento, recusarVenda, aprovarVenda, type ItemVendaEntrada } from "./vendas";
...
export async function criarVendaAction(formData: FormData): Promise<ResultadoAcao> {
  ...
  if (acao === "orcamento") {
    const resultadoEnvio = await enviarOrcamento(supabase, { tenantId: contexto.tenantId, vendaId: resultado.id });
...
export async function enviarOrcamentoAction(vendaId: string): Promise<ResultadoAcao> {
  const contexto = await obterUsuarioETenantAtual();
  if ("erro" in contexto) return { erro: contexto.erro };

  const supabase = await createClient();
  const resultado = await enviarOrcamento(supabase, { tenantId: contexto.tenantId, vendaId });
  if ("erro" in resultado) return resultado;
  revalidarVenda(vendaId);
  return { sucesso: true };
}
