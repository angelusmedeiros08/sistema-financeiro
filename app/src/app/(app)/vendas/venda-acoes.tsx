...
import { enviarOrcamentoAction, aprovarVendaAction, recusarVendaAction } from "@/lib/vendas/vendas-actions";
...
export function VendaAcoes({ vendaId, status }: { vendaId: string; status: StatusVenda }) {
  ...
  const [enviando, setEnviando] = useState<"enviar" | "aprovar" | "recusar" | null>(null);
  const [erro, setErro] = useState("");

  async function executar(acao: "enviar" | "aprovar" | "recusar") {
    setEnviando(acao);
    setErro("");
    const resultado =
      acao === "enviar" ? await enviarOrcamentoAction(vendaId) : acao === "aprovar" ? await aprovarVendaAction(vendaId) : await recusarVendaAction(vendaId);
    setEnviando(null);
      ...
      <div className="flex items-center gap-2">
        {status === "RASCUNHO" && (
          <Button type="button" variant="outline" size="sm" disabled={enviando !== null} onClick={() => executar("enviar")}>
            {enviando === "enviar" ? <Spinner size={14} className="animate-spin" /> : "Enviar orçamento"}
          </Button>
        )}
