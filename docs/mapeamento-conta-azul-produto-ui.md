...
### 2.1 Padrão visual de tela cheia (verificado ao vivo, sessão autenticada, 2026-08-13)

**Nenhuma tela de lançamento no Conta Azul é um drawer/Sheet lateral com fundo borrado.** "Nova despesa", "Detalhes da despesa" e "Informar pagamento" são todas o **mesmo padrão**: um overlay que ocupa a viewport inteira (fundo branco sólido, sem blur do conteúdo por trás), com cabeçalho fixo (título + X de fechar) e rodapé fixo (botões de ação). É visualmente indistinguível de uma navegação de página cheia — só tecnicamente é um "rollover" (parâmetro de URL) por cima da lista.

- **"Detalhes da despesa"** (abre ao clicar numa linha de Contas a Pagar/Receber) é a peça central que faltava mapear: mistura, na mesma tela, (1) as informações do lançamento com edição inline campo a campo via ícone de lápis, e (2) uma **tabela "Informações detalhadas do pagamento"** — colunas Vencimento | Forma de pagamento | Conta | Valor R$ | Juros/Multa | Desconto | Situação (badge) | Ações. Cada linha tem um dropdown **"Ações ▾"** pequeno e ancorado (não um Sheet): "Informar pagamento", "Marcar como agendado", "Editar", "Excluir". Ou seja: não existe uma tela separada de "histórico de baixas" — o histórico É a tabela principal da tela de detalhe.
- **"Informar pagamento"** (equivalente a "dar baixa") abre como a mesma tela cheia, campos Valor da parcela/Data/Forma/Conta/Valor pago/Juros/Multa/Desconto, e duas seções colapsáveis no rodapé: **Observações** e **Anexos** (botão "Inserir anexos") — confirma que o comprovante de pagamento entra exatamente nesse formulário, não em outro lugar.
- **Recorrência**: o toggle "Repetir lançamento?" abre um select "Configurações de repetição" com só "Personalizar..." (sem preset tipo "toda semana"); "Personalizar" abre um **modal pequeno e centrado** (não tela cheia, não Sheet lateral) "Recorrência": "Repetir a cada [N] [Dia(s)/Semana(s)/Mês(es)]" + "Término da recorrência" com duas opções apenas — **"Após N Ocorrências"** ou **"Dia específico"** — **não existe opção de recorrência indefinida/até-cancelar no Conta Azul**. Quando "Repetir lançamento?" está ligado, o campo "Parcelamento (À vista/Parcelado)" desaparece da Condição de pagamento e vira "Cobrar sempre no [dia do mês]" — ou seja, **no Conta Azul uma série recorrente nunca é parcelada, cada ocorrência é sempre à vista**.
- **Anexo no lançamento** (aba "Anexo" da "Nova despesa"): tabela de linhas com **Forma do anexo** (Arquivo | Link — aceita um link externo, não só upload), **Anexo** (picker/drag-and-drop), **Tipo de anexo** (Contrato | Documento Fiscal | Documento de Cobrança | Outros), **Descrição** — com botão "+ Adicionar anexo" pra múltiplas linhas.
- **Confirmações destrutivas** (ex. excluir lançamento) usam um modal pequeno de verdade — título curto, texto direto, checkbox "Concordo com a exclusão", botão vermelho — nunca a tela cheia. É o único lugar do produto onde um modal pequeno é o padrão certo.

**Implicação de design para o nosso produto**: os quatro fluxos de ação de parcela (dar baixa, renegociar, ver histórico, cancelar), hoje implementados como Radix Sheet/Dialog (drawer lateral + fundo borrado), devem virar uma única página de detalhe da parcela nesse mesmo espírito — tela cheia (ou rota própria), com o histórico de baixas como tabela na própria página, e só confirmações destrutivas pequenas (cancelar parcela) como modal de verdade.
