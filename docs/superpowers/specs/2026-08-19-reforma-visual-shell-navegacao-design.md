...
- **KPI card**: label muted pequeno → número grande em Cabinet Grotesk com `tabular-nums` → delta colorido só nele (nunca no número) — mesmo padrão já estabelecido no ciclo 1 pro card de Saldo Projetado. Confirmado visualmente em duas referências fortes (Mercury, Cruip Mosaic/Artifact): sparkline no rodapé do card comparando o período atual (linha colorida) sobreposta ao período anterior (linha cinza fina) — não só uma linha solta.
- **Ações rápidas do painel**: confirmado no Mercury — uma pílula sólida (cor primária) pra ação mais comum + várias pílulas outline/ghost ao lado pras ações secundárias, tudo numa linha horizontal logo abaixo da saudação/título. Vale adotar em `/painel` no lugar dos botões "Nova despesa"/"Nova venda" que já existem soltos em cards separados hoje.
- **Tabela densa** (referência viva do template Artifact, shadcn/Next.js — mesma base do nosso app): breadcrumb no topo, busca full-width abaixo do título, abas de filtro rápido, dropdowns de filtro específico, botão de gerenciar colunas visíveis, cabeçalho de coluna ordenável, badge duplo por linha quando a linha tem dois status independentes (ex.: status de pagamento + status de conciliação). Referência direta pra quando o TanStack Table entrar nas telas de lançamento/parcela.
- **Badge por célula, não só por linha** (confirmado visualmente no Attio): quando uma coluna representa uma classificação categórica (não um status de fluxo), o valor da célula é o próprio badge colorido — ex. uma coluna "Natureza" ou "Prioridade" mostrando `Alto`/`Médio`/`Baixo` como pill colorida direto na célula, não como texto plano com cor à parte.
- **Tag de categoria multi-matiz consistente** (confirmado no Notion): cada categoria/centro de custo pode ganhar uma cor pastel própria e consistente em todo o app (não só verde/vermelho semântico) — útil especificamente pra `Categorias financeiras`/`Centros de custo`, que hoje não têm cor própria nenhuma.
- **Cuidado**: confirmado visualmente que a identidade de marketing de um produto nem sempre reflete a UI real do produto (Ramp usa preto/branco/amarelo-chartreuse ousado no site, mas o produto real mostrado nos mockups é mais convencional, creme/branco com accent discreto; Omie tampouco mostra o dashboard real na home, só mockups de IA/WhatsApp) — reforça a prioridade que já demos à Conta Azul (autenticada, produto real) sobre qualquer marketing site.
